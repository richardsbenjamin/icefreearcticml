...
def prepare_data(
    ...
    y_train_arr = np.column_stack(y_train) if y_train else np.empty((0, 0))
    y_val_arr = np.column_stack(y_val) if y_val else np.empty((0, 0))
    y_test_arr = np.column_stack(y_test) if y_test else np.empty((0, 0))
